  ...
  getTicketInfo = (id) => {
        ...
        callbackHandler: (response) => {
          ...
          fetch.get({
            ...
            callbackHandler: (response) => {
              ...
              if (status === constants.SUCCESS)
                this.setState({ isLoading: false, userDetails: payload });
            },
          });
        },
      });
    ...
    this.setState({ isLoading: true }, () => {
      ...
      fetch.get({
        ...
        callbackHandler: (response) =>
          this.setState({ approvals: response.payload.data }),
      });
    });
  };
  ...
  handleAddComment = () => {
    ...
    if (
      ...
      this.setState({ statusChangeLoading: true }, () => {
        fetch.put({
          url:
            constants.SERVICE_URLS.TICKET_ASSIGN +
            id +
            "?emailId=" +
            selectValue +
        ...
        this.setState({ statusChangeLoading: true }, () => {
          axios({
            ...
            if (ticketStatus === "OPEN") {
              fetch.put({
                url:
                  constants.SERVICE_URLS.TICKET_STATUS +
                  id +
                  "?status=INPROGRESS",
                    ...
                    callbackHandler: (resonse) => {
                      ...
                      fetch.get({
                        ...
                        callbackHandler: (response) => {
                          ...
                          if (status === constants.SUCCESS) {
                            _state.message = "";
                            _state.ticketJourney =
                              payload.result.ticketJourneys;
                          } else {
                            _state.message = message;
                          }
                          this.setState({
      ...
      this.setState({ statusChangeLoading: true }, () => {
        fetch.post({
          ...
          callbackHandler: (response) => {
            ...
            fetch.put({
              url:
                constants.SERVICE_URLS.TICKET_STATUS + id + "?status=RESOLVED",
              callbackHandler: (resonse) => {
                this.setState({ statusChangeLoading: false });
                fetch.get({
                  url: constants.SERVICE_URLS.TICKET_DETAILING + "/" + id,
