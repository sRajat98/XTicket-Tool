...
const TicketView = (payload) => {
  ...
  const selectMapper = (mapValues) => {
    mapValues.map((option) => {
      return (
        <option key={option.departmentId} value={option.emailId}>
          {option.name}
        </option>
      );
    });
  ...
  const switchChanges = (status) => {
    ...
    const creationMonth = String(statusCreationTime.getMonth() + 1).padStart(
      2,
      "0"
    );
        ...
        return (
          <React.Fragment>
            ...
            <div className="change-text-wrapper">
              Ticket Assigned to {ticketData.assignedTo}{" "}
            </div>
          </React.Fragment>
        );
      case "OPEN":
      ...
      case "AWAITING":
        return (
          <React.Fragment>
            ...
            <div className="change-text-wrapper">
              Awaiting for approvers to approve &nbsp;
              <span className="status-time-wrapper"> | {finalDate}</span>
            </div>
          </React.Fragment>
        );
          ...
          {ticketStatusPopup === "yes" ? (
            <div className="popup-background-wrapper">
              <FeedBackView
                ...
                onFeedBackSubmit={payload.onFeedBackSubmit}
              />
            </div>
          ) : null}
          {/* {ticketStatusPopup === 'yes' ?
          ...
          <div className="ticket-details-bottom-container">
            ...
            <div className="all-tickets-wrapper">
              ...
              <div className="ticket-snapshot-wrapper">
                <div className="snapshot-name-wrapper">
                  {ticketData.displayName}
                </div>
                <div className="snapshot-time-globe-wrapper">
                  <i className="fa fa-globe" />
                  <span className="date-text">{ticketCreationTime}</span>
                </div>
                <div className="ticket-snapshot-information-wrapper">
                  <p>
                    {" "}
                    <span className="ticket-id">#{ticketData.id}</span>{" "}
                    {ticketData.subject ? ticketData.subject : null}
                  </p>
                </div>
              </div>
              {listingData.map((ticket) => {
                return ticket.id === ticketData.id ? null : (
                  <Link key={ticket.id} to={"/ticketlist/" + ticket.id}>
                    <div
                      className="ticket-snapshot-wrapper"
                      key={ticket.id}
                      onClick={() => updateTicketData(ticket.id)}
                    >
                      <div className="snapshot-name-wrapper">
                        {ticket.displayName}
                      </div>
              ...
              <div className="ticket-brief-information-wrapper">
                ...
                <div className="assigned-officer-wrapper">
                  ...
                  <div className="profile-wrapper">
                    <select
                      value={ticketData.assignedToEmailId}
                      onChange={(e) => {
                        changeSelectValue(e.target.value, ticketData.status);
                      }}
                    >
                      {allAdminUsers
                        ? allAdminUsers.map((admin) => {
                            return (
                              <option key={admin.emailId} value={admin.emailId}>
                                {admin.name}
                              </option>
                            );
                          })
                ...
                <div className="status-wrapper">
                  ...
                  {ticketData.status === "AWAITING" ? (
                    ...
                    <select
                      className="ticket-brief-status-select"
                          ...
                          <div className="approvers">
                            <span>
                              ...
                              {approval.approvalStatus === "APPROVED" ? (
                                <span id="approved" title="Request accepted">
                                  &#x2713;
                                </span>
                              ) : approval.approvalStatus === "REJECTED" ? (
                                <span id="rejected" title="Request rejected">
                                  &#10007;
                                </span>
                              ) : (
                                <img
            ...
            <div className="selected-ticket-details-wrapper">
              <div className="selected-ticket-header-wrapper">
                <div className="selected-ticket-heading-wrapper">
                  ...
                  <div className="time-wrapper">
                    <p>
                      {" "}
                      <span style={{ fontWeight: "bold" }}> SLA</span>{" "}
                      {ticketSLATime}
                    </p>
                  </div>
                </div>
              ...
              <div className="ticket-details-sub-nav-wrapper">
                <div
                  ...
                  onClick={() => isVisible("id_conversation")}
                  style={
                    display === "id_conversation"
                      ? {
                          color: "#06A99C",
                          fontSize: "11px",
                          fontWeight: "bolder",
                ...
                <div
                  className="resolution-nav-wrapper"
                  onClick={() => isVisible("id_resolution")}
                  style={
                    display === "id_resolution"
                      ? {
                          color: "#06A99C",
                          fontSize: "11px",
                          fontWeight: "bolder",
                ...
                <div
                  ...
                  onClick={() => isVisible("id_attachment")}
                  style={
                  ...
                  <div className="total-ticket-details-wrapper">
                    {displayreplybox === true ? (
                      <React.Fragment>
                        <form
                          ...
                          onSubmit={(e) => {
                            e.preventDefault();
                            replySubmitHandler("reply", ticketData.status);
                            showreplybox();
                          }}
                        >
                          <div className="comment-box-wrapper">
                            <textarea
                              ...
                              id="reply"
                              onChange={(e) => {
                                replyChangeHandler(e.target.value);
                              }}
                              cols="95"
                              rows="8"
                              placeholder="Please add a reply.&#10;Note: This will send an email to the user who raised the ticket"
                            ></textarea>
                          </div>
                          ...
                          <div className="buttons-wrapper">
                            <span>
                              ...
                              <input
                                ...
                                value="Send"
                              />
                              <button id="cancel-reply" onClick={showreplybox}>
                                Cancel
                              </button>
                            </span>
                            <span>
                    ...
                    {displaycommentbox === true ? (
                      <React.Fragment>
                        <form
                          ...
                          onSubmit={(e) => {
                            e.preventDefault();
                            replySubmitHandler("comment", ticketData.status);
                            showreplybox();
                          }}
                        >
                          <div class="comment-box-wrapper">
                            <textarea
                              ...
                              cols="101"
                              rows="15"
                              placeholder="Please add a comment.&#10;Note: This will NOT send an email to the user who raised the ticket"
                            ></textarea>
                          </div>
                          <div class="comments-buttons-wrapper">
                            <input type="submit" value="Send"></input>
                            <button id="cancel-reply" onClick={showcommentbox}>
                              Cancel
                            </button>
                          </div>
                        </form>
                    ...
                    <div className="name-wrapper">
                      <p className="username-wrapper">
                        {ticketData.displayName}{" "}
                        <span className="details-date-wrapper">
                          {" "}
                          Created on {ticketCreationTime} {}
                        </span>
                      </p>{" "}
                      <br />
                ...
                ticketData.resolution ? (
                  <React.Fragment>
                    ...
                    <div className="resolution-wrapper">
                      <form
                        id="resolution-form"
                        onSubmit={resolutionSubmitHandler}
                      >
                        <textarea
                          form="resolution-form"
                          rows="7"
                          cols="88"
                          placeholder="Enter the resolution here..."
                          onChange={(e) => {
                            resolutionChangeHandler(e.target.value);
                          }}
                        ></textarea>
                        <input
                          type="submit"
                          onClick={() =>
                            resolutionText ? shouldDisplay("yes") : null
                          }
                          value="Send"
                        />
                      </form>
                ...
                ticketJourney ? (
                  ticketJourney.length ? (
                    <React.Fragment>
                      <div className="history-container">
                        {ticketJourney.map((change) => {
                          return (
                            <div
                              className="individual-change-wrapper"
                              key={change.id}
                            >
                    ...
                    <div className="nothing-found">
                      {" "}
                      <p> NO history of the ticket found</p>{" "}
                    </div>
                  )
                ) : null
              ) : null}
              ...
              {display === "id_attachment" ? (
                ticketData.fileName ? (
                  <a
                    className="attachment-anchor"
                    href={
                      constants.SERVICE_URLS.DOWNLOAD_FILE + ticketData.fileName
                    }
                    download
                  >
                    <div className="attachment-holder">
                      {" "}
                      <img src={attachment} height="30px" width="30px" />{" "}
                      <div className="attachment-icon-holder"></div>
                      {ticketData.fileName}
                    </div>
                  </a>
                ) : (
